...
export default function CollectionList({
  ...
  async (pageNum: number, search: string) => {
    ...
    try {
      ...
      const allItems = collectAnimeList(data);

      setItems(allItems);
      setHasMore(allItems.length > 0 && !search);
  ...
  const displayItems = useMemo(() => {
    ...
    // Client-side filtering (score only)
    if (selectedScore !== "Semua Rating") {
      const minScore = parseFloat(selectedScore);
      result = result.filter(
        (it) => parseFloat(it.score?.toString() || "0") >= minScore,
      );
    }
        ...
        <div className="flex flex-wrap gap-2.5 items-center border-t border-white/5 pt-4">
          ...
          <div className="relative">
            <select
              ...
              className={`flex items-center gap-2 px-4 py-2 rounded-xl text-xs font-bold border transition-all appearance-none cursor-pointer pr-8 bg-surface-dark ${
                selectedScore !== "Semua Rating"
          ...
          {(searchQuery || selectedScore !== "Semua Rating") && (
            <button
              onClick={() => {
                setSearchQuery("");
      ...
      <div className="flex flex-col gap-12 relative min-h-[600px]">
        <div className="flex items-center gap-4">
          <h2 className="text-3xl font-black tracking-tighter">
            Collection <span className="text-primary">List</span>
          </h2>
